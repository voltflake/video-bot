...
export async function createSlideshowVideo(items: Item[]) {
  let codec = "libx264";
  ...
  for (let i = 0; i < image_count; i++) {
    ...
    await new Promise<void>((resolve) => {
      execFile("magick", ["identify", element], { encoding: "utf-8", shell: true }, (error, stdout, stderr) => {
        if (error) {
  ...
  if (image_count === 1) {
    ...
    await new Promise<void>((resolve) => {
      execFile("ffmpeg", ffmpeg_args.split(" "), { encoding: "utf-8", shell: true }, (error, stdout, stderr) => {
        if (error) {
    ...
    ffmpeg_command = ffmpeg_command.concat(
      `[0][1]xfade=transition=slideleft:duration=${transition_duration.toFixed(1)}:offset=${slide_duration.toFixed(1)}[m0];`
    );
    ...
    ffmpeg_command = ffmpeg_command.concat(
      `" -map "[m${current_filter_result}]" -c:v ${codec} -pix_fmt yuv420p -r 60 -frames ${loop_duration} videos/${timestamp}-slideshow_loop.mp4`
    );

    await new Promise<void>((resolve) => {
      execFile(
        "ffmpeg",
        ffmpeg_command.split(" ").slice(1),
        { encoding: "utf-8", shell: true },
        (error, stdout, stderr) => {
          if (error) {
